...
obj_vector* obj_parse_vector(float factor=1.0f)
{
	obj_vector *v = (obj_vector*)malloc(sizeof(obj_vector));
	v->e[0] = atof( strtok(NULL, WHITESPACE)) * factor;
	v->e[1] = atof( strtok(NULL, WHITESPACE)) * factor;
	...
	{
        float convertedValue = atof(thirdElem);
        if (factor < 1.0f) {
            v->e[2] = convertedValue * factor;
        }
        else {
            v->e[2] = atof(thirdElem) * factor;
        }
	}
...
obj_parse_obj_stream(obj_growable_scene_data* growable_data, char* bytesStream, char* materialStream)
{
	...
	while (std::getline(streamIn, current_line))
	{
		...
		{
			list_add_item(&growable_data->vertex_list, obj_parse_vector(m_resizeFactor), NULL);
		}
...
int parse_obj_scene(obj_scene_data *data_out, char *filename, float resizeFactor)
{
	obj_growable_scene_data growable_data;
